class BatchesController < ApplicationController
  ...
  def assign_employee
    @batch = Batch.find_by_id(params[:batch_id])
    @employees = Employee.find_all_by_employee_department_id(params[:department_id])
    @assigned_emps = @batch.employee_id
    @batch.update_attributes :employee_id =>@assigned_emps.to_s + "#{params[:id].to_s},"
    @assigned_employee =@assigned_emps.to_s + "#{params[:id].to_s},"
    render :update do |page|
      page.replace_html 'employee-list', :partial => 'employee_list'
  ...
  def remove_employee
    ...
    @employees = Employee.find_all_by_employee_department_id(params[:department_id])
    @assigned_emps = @batch.employee_id
    @removed_emps = @assigned_emps.gsub("#{params[:id].to_s},","")
    @batch.update_attributes :employee_id =>@removed_emps
    @assigned_employee = @removed_emps
    render :update do |page|
      page.replace_html 'employee-list', :partial => 'employee_list'
      page.replace_html 'tutor-list', :partial => 'assigned_tutor_list'
    end
