class FinanceController < ApplicationController
    ...
    def automatic_transactions
        ...
        @categories = FinanceTransactionCategory.find(:all ,:conditions => ["id != 1 and id != 3 and is_income=1 "])
    end
...
#        end
    ...
    def employee_payslip_reject
        ...
        @employee = Employee.find(params[:id])
        @salary_date = params[:id2]
        @approve = @employee.is_payslip_approved(@salary_date)
        @rejected = @employee.is_payslip_rejected(@salary_date)
        @currency_type = Configuration.find_by_config_key("CurrencyType").config_value
        ...
        @monthly_payslips = MonthlyPayslip.find_all_by_salary_date(params[:id2],
            :conditions=> "employee_id =#{params[:id]}",
            :order=> "payroll_category_id ASC")

        @individual_payslip_category = IndividualPayslipCategory.find_all_by_salary_date(params[:id2],
            :conditions=>"employee_id =#{params[:id]}",
            :order=>"id ASC")
        @individual_category_non_deductionable = 0
        @individual_category_deductionable = 0
    ...
    def additional_fees_update
        ...
        render :update do |page|
            if @finance_fee_category.update_attributes(:name =>params[:finance_fee_category][:name], :description =>params[:finance_fee_category][:description])
                @collection_date.update_attributes(:start_date=>params[:additional_fees][:start_date], :end_date=>params[:additional_fees][:end_date])
    ...
    def load_fees_submission_batch
        ...
        @student ||= FinanceFee.first(:conditions=>"fee_collection_id = #{@date.id}").student
